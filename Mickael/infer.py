...
class ASR(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        print("p_ctc[0].shape:", p_ctc[0].shape)
        # # print("p_ctc[0]:", p_ctc[0])
        # print("p_ctc[0][0].shape:", p_ctc[0][0].shape)
        # # print("p_ctc[0][0]:", p_ctc[0][0])

        for i in range(p_ctc[0].shape[0]): # for i in range(0, 26):
            # print("p_ctc[0][" + str(i) + "].shape:", p_ctc[0][i].shape)
            # input()
            id_max = 0
            for j in range(p_ctc[0][i].shape[0]):
                if p_ctc[0][i][j] > p_ctc[0][i][id_max]:
                    id_max = j
            print(id_max, end=" ")
        print()

        import pickle
        with open("analyze_ctc/p_ctc_char.pkl", "wb") as f:
            pickle.dump(p_ctc, f)
        print(type(p_ctc))
        input()
